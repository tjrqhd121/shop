...
<%@ include file="../header/head.jsp" %>
	<form id="registerForm" method="POST" >
		<div style="margin:0 auto; margin-left:27%; margin-top:50px;">
			<div>제품분류 :
				<select name="category" id="category">
					<option value="healthy" <c:if test="${read.category eq 'healthy'}">selected</c:if>>건강 보조제</option>
					<option value="object" <c:if test="${read.category eq 'object'}">selected</c:if>>스포츠 용품</option>
					<option value="mass" <c:if test="${read.category eq 'mass'}">selected</c:if>>보충제</option>
				</select>
			</div>
			<div>제품명 :
				<input type="text" name="pname" id="pname" class="pwrite" value="${read.pname}">
			</div>
			<div>판매가 :
				<input type="text" name="price" id="price" class="pwrite" value="${read.price}">
			</div>
			<div>재고 :
				<input type="text" name="quantity" id="quantity" class="pwrite" style="margin-left: 33px;" value="${read.quantity}">
			</div>
			<div style="width:70%;">내용 :
				<textarea row="5" class="form-control" name="text">${read.text}</textarea>
			</div>
			<div>파일첨부
		...
		$.getJSON("/getAttach/"+pno,function(list){
			$(list).each(function(){
...
/* 			var text1 = ${read.text};
			CKEDITOR.instances['text'].setData(text1); */
		</script>
</body>
</html>